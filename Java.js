...
// --- LOGICA DOMICILIO MULTI-PRODUCTO ---
let productosSeleccionadosDomicilio = [];

if (formDomicilio) {
    const btnAdd = document.getElementById("btnAgregarProdDom");
    const listaUI = document.getElementById("listaProductosSeleccionados");
    const select = document.getElementById("selectProductoDomicilio");
    ...
    if (btnAdd) {
        btnAdd.addEventListener("click", () => {
            ...
            if (prod) {
                productosSeleccionadosDomicilio.push(prod);
                renderizarListaDomicilio();
    ...
    function renderizarListaDomicilio() {
        if (productosSeleccionadosDomicilio.length > 0) {
            listaUI.style.display = "block";
            listaUI.innerHTML = productosSeleccionadosDomicilio.map((p, i) => `
                <li style="padding: 5px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between;">
                    ${p} 
                    <span style="color: red; cursor: pointer;" onclick="eliminarProdDomicilio(${i})">✖</span>
                </li>
            `).join('');
        } else {
    ...
    window.eliminarProdDomicilio = (index) => {
        productosSeleccionadosDomicilio.splice(index, 1);
        renderizarListaDomicilio();
    };
    ...
    formDomicilio.onsubmit = function (e) {
        ...
        // Validar que haya al menos un producto (ya sea en la lista o en el select si se le olvidó dar +)
        const prodEnSelect = select.value;
        if (prodEnSelect) {
            productosSeleccionadosDomicilio.push(prodEnSelect);
        }

        if (productosSeleccionadosDomicilio.length === 0) {
            alert("⚠️ Por favor agrega al menos un producto a tu lista de domicilio.");
            return;
        }
        ...
        const listaFinal = productosSeleccionadosDomicilio.map(p => `- ${p}`).join('\n');

        alert(`¡Domicilio Confirmado! 🛵\n\nProductos:\n${listaFinal}\n\nDatos de Envío:\nCiudad: ${ciudad}\nDirección: ${direccion}\nTeléfono: ${telefono}\n\nMétodo de Pago: ${metodo.toUpperCase()}\n\n¡Gracias por tu compra! 💖`);

        modal.style.display = "none";
        formDomicilio.reset();
        productosSeleccionadosDomicilio = [];
        renderizarListaDomicilio();
    }
}
...
// --- POPULAR SELECT DE DOMICILIO CON PRODUCTOS ---
document.addEventListener('DOMContentLoaded', () => {
    const selectDomicilio = document.getElementById('selectProductoDomicilio');
    if (selectDomicilio) {
        const nombresProductos = [];
        document.querySelectorAll('.producto h3').forEach(h3 => {
            nombresProductos.push(h3.textContent);
        });

        // Eliminar duplicados si hay
        const unicos = [...new Set(nombresProductos)];

        unicos.sort().forEach(nombre => {
